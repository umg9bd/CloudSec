...
feature_engine8.py
===================
Two correctness fixes on top of fe7, found while reviewing fe7 against real
CloudTrail logs (datasets/privilege-escalation/stratus_own_runs/CloudTrail/):

  A. access_key_id was only read from top-level access_key_id/accessKeyId
     fields. Real CloudTrail JSON doesn't put accessKeyId at the top level --
     it's nested under userIdentity.accessKeyId. Every other identity field
     (principal_arn, principal_type, mfa_authenticated) already fell back to
     the nested userIdentity/sessionContext lookup; accessKeyId was the one
     field that didn't, so has_access_key was silently 0 for every row on
     real logs (it only ever worked because invictus_enriched.csv happens to
     have a flattened access_key_id column already).

  B. get_temporal_features crashed with a bare TypeError on a row with no
     timestamp/eventTime at all ('T' in None). Malformed/incomplete records
     do show up in real log files; one bad row shouldn't take down an entire
     batch. Such rows are now skipped (with a warning printed) instead of
     killing the whole run.

Also added: 'username' and raw 'timestamp' columns on every temporal.csv row
(TEMP_FIELDS), for a downstream LSTM script to group by username and build
per-user sliding windows without re-joining against the structural CSV or
the original input file. These are raw metadata, not model input features --
they are not added to TEMPORAL_COLS, which still holds only the normalized
numeric feature vector. Two follow-up fixes for that same LSTM use case:

  C. invictus_temporal.csv is now kept globally sorted by (username,
     timestamp) -- see _rewrite_temporal_sorted -- instead of just file-
     arrival order. CloudTrail doesn't guarantee event order within a
     delivered log file, let alone across files, and --watch mode files can
     land out of chronological order; without this, a per-user sliding
     window built by reading the CSV top-to-bottom could silently see
     events out of sequence.

  D. StateTracker (velocity/session state per principal) is now persisted
     to .state_tracker.json, the same way the event_name vocab already was.
     Previously a --watch restart reset every principal's action_velocity /
     is_new_action / session_duration_normalized back to "brand new," even
     mid-session -- silently truncating exactly the kind of cross-restart
     session continuity a sliding-window LSTM is meant to pick up on.

fe7's checklist (unchanged, still in effect):

  1. event_name / event_source / principal_type get a categorical VOCAB
     INDEX (event_name_idx, event_source_idx, principal_type_idx) alongside
     the existing hand-picked risk scores, instead of relying on the hand
     mapping alone. A downstream model can embed these indices
     (nn.Embedding) and learn its own representation on top of the MITRE-
     informed prior, rather than being stuck with only one or the other.
     principal_type and event_source use a FIXED vocab (AWS's real
     documented principal types; the ~9 services this project's Stratus
     techniques actually touch) with an explicit <UNK> bucket for anything
     outside it. event_name uses a persisted, growable vocab
     (.event_name_vocab.json) since AWS's API surface is too large to hand-
     curate -- new action names get appended to it as they're seen.

  2. Action velocity now also tracks SESSION duration and events/minute,
     not just the single inter-event time delta (see StateTracker).

  3. New policy/permission features parsed from request_params_raw's nested
     policyDocument / assumeRolePolicyDocument JSON: statement count,
     wildcard Action/Resource presence, and a privileged-action-reach score
     (also catches AdministratorAccess-style policyArn attachments, which is
     the actual pattern in this project's synthetic data).

  4. read_only / mfa_authenticated are no longer silently imputed to
     'true'/'false' when absent from the source log -- absence is kept as
     its own explicit signal (mfa_absent / read_only_absent). This matters:
     mfa_authenticated is missing on 77% of the real invictus_enriched.csv
     rows (AWS service principals structurally lack MFA), so imputing
     'false' there was manufacturing a false "no MFA" signal for rows where
     we simply don't know.

  5. All new continuous features are bounded to [0, 1] via the same
     min(x / cap, 1.0) convention fe4-6 already used for params_length, so
     the temporal branch stays on a consistent scale.

Ingestion / micro-batch / fast-lane infrastructure is unchanged from fe6/fe7.

NOTE: TEMP_FIELDS gained 'username'/'timestamp' columns, so the temporal
output schema changed from fe7. Delete any existing invictus_temporal.csv
(and its structural sibling, to keep log_id numbering in sync) before the
first run, or this script will refuse to append mismatched rows to it.

Run:
    python feature_engine8.py
        One-shot batch over invictus_enriched.csv.

    python feature_engine8.py --watch incoming/ [--simulate]
        Same watch-folder mode as fe6/fe7.
"""
...
def normalize_cloudtrail_row(row):
    ...
    # the source log, instead of defaulting to a guessed value -- see
...
class FeatureEngineer:
    def __init__(self, event_name_vocab_path=None, state_tracker_path=None):
...
def _check_output_schema(path, expected_fields):
    ...
    already in the file -- the temporal schema changed from fe6."""
    ...
    if existing_header is not None and existing_header != expected_fields:
        raise SystemExit(
            f"{path} has a different column layout than feature_engine8 produces.\n"
...
def _rewrite_temporal_sorted(new_rows) -> None:
    """Keeps invictus_temporal.csv globally sorted by (username, timestamp),
    not just in file-arrival order.
    ...
    into place, rather than truncating invictus_temporal.csv directly --
...
def run_batch(input_path: str) -> None:
    if not os.path.exists(input_path):
        ...
        raise SystemExit(1)
    engine = FeatureEngineer(event_name_vocab_path=EVENT_NAME_VOCAB_FILE, state_tracker_path=STATE_TRACKER_FILE)
