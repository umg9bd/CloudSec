...
def node_key_for_target(target_value: str, target_resource_type: str, target_service: str) -> GraphNodeKey:
    ...
    role_name = role_name_from_iam_arn(value)
    if role_name:
        return GraphNodeKey("Role", role_name)

    if ":policy/" in value:
        return GraphNodeKey("Policy", value)

    # The current structural dataset uses role-* as the
    # canonical identifier for IAM role targets.
    if target_resource_type == "opaque" and value.startswith("role-"):
        return GraphNodeKey("Role", value)

    if target_resource_type == "service-domain":
        return GraphNodeKey("Service", value)

    return GraphNodeKey("Resource", value)
# ══════════════════════════════════════════════════════════════════════════
