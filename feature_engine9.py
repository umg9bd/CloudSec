...
def normalize_cloudtrail_row(row):
    ...
    if request_params is None:
        request_params_obj = row.get('requestParameters') or {}
        if isinstance(request_params_obj, (dict, list)):
            request_params = json.dumps(request_params_obj)
        else:
            request_params = str(request_params_obj)
    else:
        request_params_obj = _parse_json_text(request_params) or {}

    if not isinstance(request_params_obj, dict):
        request_params_obj = {}

    target_resource = row.get('target_resource')
    if not target_resource:
        event_name = row.get('event_name') or row.get('eventName') or ''

        # Extract AWS account ID from the principal ARN when we need
        # to construct canonical IAM ARNs for role/user names.
        account_id = ""
        if principal_arn:
            arn_parts = str(principal_arn).split(":")
            if len(arn_parts) > 4 and arn_parts[4].isdigit():
                account_id = arn_parts[4]

        if event_name == 'AssumeRole':
            # Canonical IAM role ARN
            target_resource = request_params_obj.get('roleArn')

        elif event_name in ('CreateRole', 'UpdateAssumeRolePolicy'):
            role_name = request_params_obj.get('roleName')
            if role_name:
                target_resource = (
                    f"arn:aws:iam::{account_id}:role/{role_name}"
                    if account_id else role_name
                )

        elif event_name in ('AttachRolePolicy', 'PutRolePolicy'):
            # The role being modified is the primary graph target.
            role_name = request_params_obj.get('roleName')
            if role_name:
                target_resource = (
                    f"arn:aws:iam::{account_id}:role/{role_name}"
            ...
            # Canonical IAM policy ARN
            target_resource = request_params_obj.get('policyArn')

        elif event_name == 'CreateAccessKey':
            user_name = request_params_obj.get('userName')
            if user_name:
                target_resource = (
                    f"arn:aws:iam::{account_id}:user/{user_name}"
                    if account_id else user_name
                )

        elif event_name == 'PutBucketPolicy':
            bucket_name = request_params_obj.get('bucketName')
            if bucket_name:
                target_resource = f"arn:aws:s3:::{bucket_name}"

        elif event_name == 'GetSecretValue':
            target_resource = request_params_obj.get('secretId')

        elif event_name in ('StopLogging', 'DeleteTrail'):
            target_resource = request_params_obj.get('name')

        # Existing CloudTrail resources fallback
        if not target_resource:
            ...
            if isinstance(resources, list) and resources:
                ...
                if isinstance(first_resource, dict):
                    target_resource = (
                        first_resource.get('ARN')
                        or first_resource.get('arn')
                        or first_resource.get('name')
                    )
    ...
    print(
        f"[DEBUG] {normalized['event_name']}: "
        f"{normalized['principal_arn']} -> "
        f"{normalized['target_resource']}"
    )
