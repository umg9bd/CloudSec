"""Production P_seq scorer — LSTM–Transformer v5 (max-pool events → windows)."""
...
def load_scorer(
    ...
    if not path.exists():
        raise FileNotFoundError(
            f"Missing checkpoint {path}. Train with train_lstm_transformer.py"
        )
    ...
    if not vocab and tlt.VOCAB_PATH.exists():
        import json
...
def score_dataframe(df: pd.DataFrame, scorer: Scorer) -> pd.DataFrame:
    """Validate → per-event v5 score → P_seq = max(P_event) on 10-min / stride-2 windows."""
    try:
