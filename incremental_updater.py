...
# BUG FOUND DURING AUDIT (2026, HGT/sampling extension — see FINAL_REPORT.md):
# neo4j_graph_builder.py physically lives in graph_construction/, one
# directory below this file, but this module has always imported it as a
# bare top-level module. That only resolves if graph_construction/ happens
# to be on sys.path already (e.g. cwd), which is not true when this file is
# imported as a package/module (`python -m pytest`, `import
# incremental_updater`, infer.py's own import of this module, etc.) —
# verified directly: `python3 -c "import incremental_updater"` raised
# ModuleNotFoundError: No module named 'neo4j_graph_builder' before this
# fix. Adding graph_construction/ to sys.path here (mirroring the same
# pattern graph_construction/neo4j_graph_builder.py already uses in the
# opposite direction to import privilege_features) is the minimal fix —
# no files moved, no interfaces changed.
