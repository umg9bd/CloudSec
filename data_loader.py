...
class PrivilegePropagationGraphLoader:
    ...
    def load(self) -> Tuple[HeteroData, dict]:
        ...
        for (src_type, rel, dst_type), df in combined.groupby(["src_type", "relation", "dst_type"], sort=True):
            ...
            # integer sort did.
            df = df.sort_values("log_id").reset_index(drop=True).copy()
            triple = (src_type, rel, dst_type)

            df.loc[:, "src_idx"] = df["src_key"].map(node_idx[src_type])
            df.loc[:, "dst_idx"] = df["dst_key"].map(node_idx[dst_type])
            ...
            df.loc[:, "src_idx"] = df["src_idx"].astype(int)
            df.loc[:, "dst_idx"] = df["dst_idx"].astype(int)

            edge_index = torch.tensor(
                np.stack([df["src_idx"].values, df["dst_idx"].values]),
                dtype=torch.long
            )
        ...
        for ntype in PRINCIPAL_NODE_TYPES:
            ...
            if df is not None and "is_known_attacker_identity" in df.columns:
                attacker_identity_by_key.update({
                    key: bool(value) if value is not None else False
                    for key, value in zip(
                        df["key"],
                        df["is_known_attacker_identity"],
                    )
                })
    ...
    def _fetch_edges(self, relation: str) -> pd.DataFrame:
        ...
        if len(df):
            ...
            df.loc[:, "src_type"] = df["src_labels"].apply(get_specific_label)
            df.loc[:, "dst_type"] = df["dst_labels"].apply(get_specific_label)
            df.loc[:, "is_read_only"] = df["edge_type"].str.startswith(
    ...
    def _node_features(self, ntype: str, df: pd.DataFrame) -> torch.Tensor:
        ...
        if "distance_to_sensitive_resource" in df.columns:
            distance = pd.to_numeric(
                df["distance_to_sensitive_resource"],
                errors="coerce",
            )

            df.loc[:, "distance_to_sensitive_resource"] = distance.fillna(
    ...
    def _edge_features(
        ...
        df = df.assign(
            ...
            privilege_gain=df["privilege_gain"].fillna(0.0),
        )
