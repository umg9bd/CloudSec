...
def evaluate(
    ...
    metrics = {
        ...
        # AUPR (average precision) — added: task requirement 11 explicitly
        # lists this alongside precision/recall/F1/AUROC as a required
        # ablation metric, and it was missing from this function entirely
        # (found during audit). More informative than AUROC on this
        # dataset's class imbalance (~4.9% positive) — AUROC can look
        # deceptively high under heavy imbalance since it's dominated by
        # the (easy) majority class's ranking, while AUPR is sensitive to
        # precision specifically on the rare positive class, which is
        # what an edge-level attack detector is actually judged on.
