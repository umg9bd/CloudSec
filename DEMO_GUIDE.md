...
accuracy on individual actions is not just weak but inverted (AUC≈0.26) — an
open question you should raise yourself rather than be caught by. See "What to say"
...
INFO | Acc=0.99  P=0.95-0.98  R=0.90-0.95  F1=0.93-0.95  AUC=0.999  (n=1480)
```
...
**Say when this finishes:** "This F1≈0.95 is on synthetic held-out data —
...
SUMMARY: P=0.214  R=0.059  F1=0.092  AUC=0.260

NOTE: this is an EDGE-level score. Do not compare it against the rule baselines,
which are session-level. Run evaluate_session_level.py for a like-for-like comparison.
```

**Say:** "Edge-level, the model is not just weak — it's *inverted*. AUC=0.26
means real attack edges score lower than real benign ones when you pool every
edge together. We report that rather than hide it; it's an open question we
flag in the paper. It is emphatically not an edge-level detector. That's
...
```powershell
python evaluate_session_level.py --checkpoint checkpoints/best_GraphSAGE_wrapped.pt --model sage --raw-csv datasets/privilege-escalation/real_dataset_test.csv --threshold 0.50
```
...
**Rule-based baselines** — note this script reports on `real_dataset_combined.csv`
(all 397 dev+test sessions), **not** the 238-session test split the model is
scored on:
```powershell
...
the same rule set scores **F1=0.747** [0.667, 0.811]. `evaluate_session_level.py`
now computes the baseline on the exact sessions it just scored and prints a
paired bootstrap, so just read the comparison off that output rather than
citing a remembered number.
