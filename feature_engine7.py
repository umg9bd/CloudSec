...
feature_engine7.py
===================
Fixes to fe6's feature set, from a checklist review:
...
Ingestion / micro-batch / fast-lane infrastructure is unchanged from fe6.

Run:
    python feature_engine7.py
        One-shot batch over invictus_enriched.csv.

    python feature_engine7.py --watch incoming/ [--simulate]
        Same watch-folder mode as fe6.

NOTE: the temporal output schema changed from fe6 (more columns). Delete
any existing invictus_temporal.csv before the first run, or this script
will refuse to append mismatched rows to it.
"""
...
class FeatureEngineer:
    def __init__(self, event_name_vocab_path=None):
...
STATE_FILE = os.path.join(DATA_DIR, ".feature_engine7_state.json")
...
def _check_output_schema(path, expected_fields):
    ...
    if existing_header is not None and existing_header != expected_fields:
        raise SystemExit(
            f"{path} has a different column layout than feature_engine7 produces.\n"
...
def run_batch(input_path: str) -> None:
    if not os.path.exists(input_path):
        ...
        raise SystemExit(1)
    engine = FeatureEngineer(event_name_vocab_path=EVENT_NAME_VOCAB_FILE)
