...
# neo4j_graph_builder.py lives in graph_construction/, a sibling directory of
# this file (infer.py sits in the parent folder alongside graph_construction/).
# It's imported both directly below (`import neo4j_graph_builder as nb`) and
# transitively by incremental_updater.py (`import neo4j_graph_builder as nb`),
# so graph_construction/ is added to sys.path here, before either import,
# rather than in each importing module individually.
import sys as _sys
_sys.path.insert(0, str(Path(__file__).resolve().parent / "graph_construction"))
...
class InferenceEngine:
    ...
    def __init__(
        ...
        # all live under feature_engine9.DATA_DIR. feature_engine9's own
        # batch/watch entry points (process_batch_file, watch_folder) always
        # os.makedirs(DATA_DIR) before writing there, but InferenceEngine
        # constructs FeatureEngineer directly and previously skipped that,
        # so a fresh checkout with no datasets/privilege-escalation/ folder
        # would run fine until shutdown, when save_state() tried to write
        # into a directory that never got created. Ensure it exists here too.
        os.makedirs(os.path.dirname(EVENT_NAME_VOCAB_FILE), exist_ok=True)
