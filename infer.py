...
#
# BUG FOUND DURING AUDIT: neo4j_graph_builder.py lives in graph_construction/,
# not at top level — this bare import only ever worked by accident, via
# incremental_updater.py (imported above) having already inserted
# graph_construction/ onto sys.path as a side effect. Made explicit and
# self-sufficient here too, rather than depending on import order elsewhere
# in this file staying exactly as-is. See incremental_updater.py's own
# comment at its equivalent sys.path.insert for the full explanation.
import sys as _sys
_sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), "graph_construction"))
...
def load_model_from_checkpoint(ckpt_path: str, device: torch.device) -> Tuple[torch.nn.Module, dict]:
    ...
    Expects the checkpoint to be a dict with keys:
        "state_dict"   : model.state_dict()   (unused when model_type="ensemble" —
                          see below)
    ...
    model_args["model_type"] selects which model gets constructed:
        "graphsage" (default when the key is absent — every checkpoint
                     from before this option existed loads through
                     exactly this same branch, unchanged)
        "hgt"       — see model_hgt.py
        "ensemble"  — see model_ensemble.py; expects model_args["ensemble"]
                     with each component's own self-contained state_dict
                     + model_args (the checkpoint's top-level "state_dict"
                     key is unused in this case, since there's no single
                     flat state_dict for an ensemble of models)
    ...
    model_type = args.get("model_type", "hgt")  # absent key = old checkpoint = this exact branch
    if model_type == "graphsage":
        ...
        from model_hgt import build_hgt_from_args  # local import: existing graphsage-only
        model = build_hgt_from_args(args)           # path never needs model_hgt.py to import cleanly
        model.load_state_dict(ckpt["state_dict"])
    elif model_type == "ensemble":
        from model_ensemble import build_ensemble_from_args  # local import, same reasoning
        model = build_ensemble_from_args(args, device)  # loads its own component state_dicts internally
    else:
        raise ValueError(
            f"Checkpoint at {ckpt_path!r} has unknown model_args['model_type'] "
            f"{model_type!r} — expected 'graphsage', 'hgt', or 'ensemble'."
        )
    model.to(device)
    model.eval()
    n_params = sum(p.numel() for p in model.parameters())
    if model_type == "ensemble":
        component_names = [c.get("name", c["model_type"]) for c in args["ensemble"]["components"]]
        log.info("Model loaded (model_type=ensemble, components=%s) — %d parameters", component_names, n_params)
    else:
        log.info(
            "Model loaded (model_type=%s) — %d parameters | edge_types=%d | edge_feat_dim=%d",
            model_type, n_params, len(args["edge_types"]), args["edge_feat_dim"],
        )
...
""",
    ...
    # Model / checkpoint
    p.add_argument("--checkpoint", default="./checkpoints/best_HGT_wrapped.pt",
                   help="Path to the production HGT checkpoint (default: %(default)s).")
    ...
    p.add_argument(
    "--model-type",
    choices=["graphsage", "gat", "hgt"],
    default="hgt",
    help="Architecture of the checkpoint. HGT is the production default."
    )
    ...
    p.add_argument(
        ...
        type=int,
        default=4,
        help="Number of attention heads used by HGT/GAT."
    )
    return p.parse_args()
